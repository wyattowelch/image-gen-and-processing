...
def scanSubImage(img_gray, img, pixel_indices, slope, sun_vec):
    ...
    for li, line in enumerate(pixel_indices):
        ...
        # Filter edge widths to remove +/- 2 sigma outliers and widths 
            # Using median rather than average to handle outliers

        w = np.asarray(edge_widths, dtype=float)
        intercepts = np.asarray(intercept_indices, dtype=int) 
        assert w.shape[0] == intercepts.shape[0], "Widths and intercepts arent the same number of elements"
        ...
        lo = med - 2*sigma
        hi = med + 2*sigma
        mask_width = (w >= lo) & (w <= hi)

        cropped_edge_widths = w[mask_width]
        cropped_intercepts = intercepts[mask_width]


    cv2.imwrite('scanImageTest.png', image)


    return cropped_intercepts, cropped_edge_widths


# ---------------------### Zernike Mask for Sub Pixel Edge Detection ### ----------------------------------
def zernike_mask(N, samples=60):
    p = (N - 1) // 2
    step = 2 / N
    du = step / samples
    dv = step / samples

    # midpoints of subpizel (and offset)
    t = (np.arange(samples) + .5) / samples - .5
    offset = t * step
    
    M11_re = np.zeros((N,N), dtype=float)
    M20 = np.zeros((N,N), dtype=float)
    ...
    for dy in range(-p, p + 1):
        for dx in range(-p, p + 1):
            ...
            # eq 50/51 of Christian and Robinson

            int_u = np.sum(u * inside) * du * dv
            int_20 = np.sum((2 * (u**2 + v**2) - 1) * inside) * du * dv

            M11_re[dy + p, dx + p] = int_u
            M20[dy + p, dx + p] = int_20
...
def sub_pixel_edge(N_size, edge_pts55, edge_coor55, edge_widths, mask_samples=60, eps=1e-6, w_eps=1e-6):
    ### -------- INPUTS -------- ###
    ...
    # Zernike Function
    M11_re, M20 = zernike_mask(N_size, samples=mask_samples)
    ...
    for k in range(n_edges):
        ...
        # Moments from eq 76-79
        reA11 = float(np.sum(patch * M11_re))
        imA11 = float(np.sum(patch * M11_re.T))
        A11 = reA11 + 1j * imA11
        A20 = float(np.sum(patch * M20))

        if abs(A11) < eps:
            sub_edge_coor[:, k] = [row_c, col_c, 1]
            continue
        # eq 56
        psi = np.arctan2(A11.imag, A11.real)

        # eq 60
        A0_11 = (A11.real * np.cos(psi) + A11.imag * np.sin(psi))
        if abs(A0_11) < eps:
            sub_edge_coor[:, k] = [row_c, col_c, 1]
            continue
        ...
        # normalize half width from edge width
        w = float(edge_widths[k])
        if abs(w) < w_eps:
            l = float(ratio)
        else: 
            w2 = w * w
            # eq 66
            disc = (w2 - 1) ** 2 - 2 * w2 * float(ratio)    
            ...
            if disc < 0:
                ...
                l = (1 - w2 - np.sqrt(disc)) / w2

        l = float(np.clip(1, -1, 1))

        # eq 70
        du_px = (N_size * l / 2) * np.cos(psi)
        dv_px = (N_size * l / 2) * np.sin(psi)

        u = u_tilde + du_px
        v = v_tilde + dv_px

        sub_edge_coor[:, k] = [u, v, 1]
